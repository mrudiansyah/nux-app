    ...
    <script type="text/javascript"> 
        $(document).ready(function(){   
            ...
            if(ref_doc == '' || ref_doc == null){ 
                ...
                window.history.pushState('', '', '<?php echo env('BASE_URL') ?>/po_approval');    
            }else{ 
                $('#temp_id').val(ref_doc);  
                                ...
                                <div class="card-header border-0 pt-6"> 
                                    <div class="card-title"> 
                                        <div class="d-flex align-items-center position-relative my-1"> 
                                            ...
                                            <input type="text" data-kt-goodreceive-table-filter="search" id="front_table_search" class="form-control form-control-solid w-250px ps-15  text-sm form-control-sm" placeholder="Search PO Number" />
                                        </div> 
                                    </div> 
                                    <div class="card-toolbar"> 
                                        <button type="button" id="btn_export_excel"
                                    class="btn btn-light-success btn-sm p-0 me-2"
                                    title="Export Excel"
                                            ...
                                            <div class="menu menu-sub menu-sub-dropdown w-300px w-md-325px" data-kt-menu="true" id="kt-toolbar-filter">
                                                ...
                                                <div class="px-7 py-5"> 
                                                    <div class="mb-5"> 
                                                        ...
                                                        <select class="form-select form-select-solid fw-bolder fw-bolder form-select-sm text-sm" data-kt-select2="true" data-placeholder="Select option" data-allow-clear="false" id="status_id" data-hide-search="true" onchange="resetFrontCard()"> 
                                                                <option class="text-sm" value='1'>Waiting Check</option> 
                                                                <option class="text-sm" value='2'>Waiting Approve</option> 
                                                                <option class="text-sm" value='3'>Waiting Legalize</option> 
                                                                <option class="text-sm" value='4' selected>All Status</option> 
                                                        </select> 
                                                    </div> 
                                                    <div class="mb-5"> 
                                                        <label class="form-label fs-5 fw-bold mb-3">Buyer:</label> 
                                                        <select class="form-select form-select-solid fw-bolder form-select-sm text-sm" data-kt-select2="true" data-allow-clear="false" id="section_id" data-hide-search="true"> 
                                                                <option class="text-sm" value='' selected>ALL Buyer</option> 
                                                                <option class="text-sm" value='PPIC'>PPIC</option> 
                                                                <option class="text-sm" value='REGULER'>REGULER</option> 
                                                                <option class="text-sm" value='PROJECT'>PROJECT</option> 
                                                        </select> 
                                                    </div> 
                                                    <div class="mb-5"> 
                                                        <label class="form-label fs-5 fw-bold mb-3">Start Date</label> 
                                                        <input type="date" class="form-control form-control-solid fw-bolder fw-bolder form-control-sm text-sm" id="start_date">
                                                    </div> 
                                                    <div class="mb-5"> 
                                                        <label class="form-label fs-5 fw-bold mb-3">End Date</label> 
                                                        <input type="date" class="form-control form-control-solid fw-bolder fw-bolder form-control-sm text-sm" id="end_date">
                                                    </div> 
                                                    <div class="d-flex justify-content-end">
                                                        ...
                                                        <button type="submit" id="submit-filter" class="btn btn-primary btn-sm tex-sm" data-kt-menu-dismiss="true" data-kt-goodreceive-table-filter="filter">Apply</button>
                                                    </div>
                                                </div>
                                            </div>   
                                    ...
                                    <table class="table align-middle table-row-dashed table-striped fs-7 gy-3" id="kt_doc_table">
                                        <thead>
                                            <tr class="text-start text-gray-900 fw-bolder fs-8 text-uppercase gs-0">
                                                ...
                                                <th class="min-w-100px">PO Num</th>
                        ...
                        <div class="modal-body scroll-y mx-5 mx-xl-18 pt-0 pb-15"> 
                            ...
                            <div class="d-flex flex-stack p-14">   
                                <label class="form-check form-check-custom form-check-solid ml-16">
                                    <input class="form-check-input approval_checkbox" type="checkbox" value="APPROVED" onclick="checkboxAction(this)"/>
                                    <span class="form-check-label text-gray-600">Approve</span>
                                </label> 
                                <label class="form-check form-check-custom form-check-solid mr-16">
                                    <input class="form-check-input approval_checkbox" type="checkbox" value="REJECTED" onclick="checkboxAction(this)"/>
...
<script>
    ...
    function sendEmail() {  
            ...
            $.ajax({
                type	: 'POST',
                url	: "{{ route('po_approval.send_email_po') }}",
                data	: string,
                cache	: false,
                dataType	: 'json',
    ...
    function submitApproval() {
        ...
        var string = "&_token="+token+"&trc_unix_id="+temp_id+"&approve_sts="+approve_sts+"&approve_msg="+approve_msg ;
            $.ajax({
                type	: 'POST',
                url	: "{{ route('po_approval.submit_approval') }}",
                data	: string,
                cache	: false,
                dataType	: 'json',
                success : function(data){     
    ...
    function getCountDocument() {
        ...
        $.ajax({
            type	: 'POST',
            url	: "{{ route('po_approval.get_count_document') }}",
            data	: string,
            cache	: false,
            dataType	: 'json',
            success : function(data){   
    ...
    function getPreview() {
        ...
        $.ajax({
            type	: 'POST',
            url	: "{{ route('po_approval.print_view') }}",
            data	: string,
            cache	: false,
            success : function(data){  
                setTimeout(function(){
    ...
    function getBApprove() {  
        ...
        $.ajax({
            type	: 'POST',
            url	: "{{ route('po_approval.get_button_approve') }}",
            data	: string,
            cache	: false,
            dataType	: 'json',
            success : function(data){   
    ...
    function getAttachmentList() { 
        ...
        $.ajax({
            type	: 'POST',
            url	: "{{ route('po_approval.get_attachment_list') }}",
            data	: string,
            cache	: false,
            success : function(data){ 
    ...
    function getCommentList() { 
        ...
        $.ajax({
            type	: 'POST',
            url	: "{{ route('po_approval.get_comment_list') }}",
            data	: string,
            cache	: false,
            success : function(data){  
                setTimeout(function(){
    ...
    function sentComment() {     
        ...
        $.ajax({
            type	: 'POST',
            url	: "{{ route('po_approval.sent_comment') }}",
            data	: string,
            cache	: false,
            dataType : 'json',
            success : function(data){  
    ...
    $(document).ready(function () {
        var frontTable = $("#kt_doc_table").DataTable({
            ...
            ajax: {
                url: "{{ route('po_approval.front_table') }}",
                type: 'POST',
                data	: function ( d ) { d._token = $("[name=_token]").val(), d.status_id = $("#status_id").val(), d.section_id = $("#section_id").val(), d.front_table_search = $("#front_table_search").val(),d.start_date = $("#start_date").val(), d.end_date=$("#end_date").val(); }, 
                cache	: false,
                dataType : 'json'
            },
            columns: [
        ...
        $("#submit-filter").click(function(){
                frontTable.ajax.reload(); getCountDocument(); 
        }); 
    }) ;
    ...
    {      
        ...
        $.ajax({
            ...
            success : function(data){   
                ...
                setTimeout(function(){
                    $("#lds-roller").css("display", "none");  
                    window.history.pushState('', '', '<?php echo env('BASE_URL') ?>/po_approval?ref_doc='+trc_unix_id); 
                },500)
        } }) 
    };
...
function exportExcel(ev) {
    ...
    const url = `{{ route('po_approval.export_excel') }}?` + params.toString();
