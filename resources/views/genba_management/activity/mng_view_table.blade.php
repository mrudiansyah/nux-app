...
@section('subcontent')
                                        ...
                                        <div class="px-7 py-5">
                                            ...
                                            <div class="mb-5">
                                                ...
                                                <input type="text" class="form-control form-control-solid"
                                                    id="auditor_filter" placeholder="Nama Auditor">
                                            </div>
                                            <div class="d-flex justify-content-end">
                                                <button type="button" class="btn btn-secondary me-3"
                                                    data-kt-menu-dismiss="true" id="reset-filter">Reset</button>
                        ...
                        <div class="card">
                            <div class="card-header card-header-stretch">
                                <div class="card-title d-flex align-items-center">
                                    {{-- <button class="btn btn-primary btn-sm text-sm" style="width: 100px;"
                                        onclick="getApprovalForm()">Approve</button> --}}
                                    ...
                                    <ul class="nav nav-tabs nav-line-tabs nav-stretch fs-6 border-0 fw-bolder"
                                        ...
                                        <li class="nav-item" role="presentation" onclick="getCommentList()">
                                            <a id="kt_activity_comment_tab"
                                                class="nav-link tab_preview justify-content-center text-active-gray-800"
                                                data-bs-toggle="tab" role="tab" href="#kt_activity_comment">Comment</a>
                                        </li>
                                    </ul>
                                </div> --}}
                            </div>
                                ...
                                <div class="tab-content">
                                    ...
                                    <div id="kt_activity_comment" class="card-body tab_preview p-0 tab-pane fade show"
                                        ...
                                        <div class="card-footer pt-4" id="kt_drawer_chat_messenger_footer">
                                            <textarea id="input_comment" class="form-control form-control-flush mb-3"
    ...
    <script>
        ...
        //     });
        // }
        // Function 1: Menampilkan Konfirmasi Yes/No
        function confirm_delete_doc(id, no) {
            // Definisi ID elemen tombol agar spesifik
            var btn = $('#btn_delete_action_' + no);
            var icon = $('#icon_delete_' + no);
            var loader = $('#loader_delete_' + no);
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    // Jika User klik YA -> Lanjut ke proses hapus
                    execute_delete_doc(id, no);
                } else {
                    // Jika User klik BATAL -> Balikkan tombol seperti semula
                    icon.show();
                    loader.hide();
        ...
        // Function 2: Proses Eksekusi Hapus ke Server (AJAX)
        function execute_delete_doc(id, no) {
            var btn = $('#btn_delete_action_' + no);
            var icon = $('#icon_delete_' + no);
            var loader = $('#loader_delete_' + no);
            var token = $("[name=_token]").val();
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('genba.delete_mng_genba') }}", // Pastikan route ini sesuai
                data: {
                    _token: token,
                    trc_unix_id: id
                },
                ...
                success: function (response) {
                    // Jika Server merespon sukses (sesuaikan dengan format response controller Anda)
                ...
                error: function (xhr, status, error) {
                    Swal.fire(
                        'Error!',
                        'Server tidak merespon: ' + error,
                        'error'
                    );
                },
                complete: function () {
                    // Apapun hasilnya, kembalikan tombol ke kondisi awal
                    icon.show();
                    loader.hide();
                    btn.prop('disabled', false);
                }
        ...
        function f_genba_conform_delete(id, no) {
            // Sesuaikan selector dengan ID baru
            var btn = $('#btn_f_genba_conform_delete_' + no);
            var icon = $('#icon_f_genba_conform_delete_' + no);
            var loader = $('#loader_f_genba_conform_delete_' + no);
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    execute_delete_doc(id, no); // Panggil fungsi eksekusi (nama function ini bebas)
                } else {
                    // Balikkan tombol jika batal
                    icon.show();
                    loader.hide();
                    btn.prop('disabled', false);
                }
        ...
        function execute_delete_doc(id, no) {
            // Selector juga harus disesuaikan di sini
            var btn = $('#btn_f_genba_conform_delete_' + no);
            var icon = $('#icon_f_genba_conform_delete_' + no);
            var loader = $('#loader_f_genba_conform_delete_' + no);
            var token = $("[name=_token]").val();
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('genba.delete_mng_genba_dtl') }}", // Sesuaikan route ke controller baru
                data: {
                    _token: token,
                    trc_unix_id: id
                },
