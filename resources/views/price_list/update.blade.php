...
<div id="kt_activity_detail" class="card-body p-0 tab-pane fade show active" role="tabpanel"
...
<script>
    ...
    $("#btnSubmitHeader").on('click', function() {
        ...
        $.ajax({
            url: "{{ url('price_list/update_header') }}",
            method: "POST",
            dataType: 'json',
            data: {
                _token: "{{ csrf_token() }}",
                id: new URLSearchParams(window.location.search).get('ref_doc'),
            ...
            success: function(response) {
                if (response.status == true) {
                    form_header(new URLSearchParams(window.location.search).get('ref_doc'))
                    Toast.fire({
                        position: 'top-end',
    ...
    function partDelete(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ url('price_list/delete_part') }}",
                    method: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        id: id
                    ...
                    success: function(response) {
                        if (response.status == true) {
                            ...
                            Swal.fire({
                                icon: 'error',
                                title: response.message ||
                                    'Failed to delete part from price list',
                                timer: 1500,
                                showConfirmButton: false
                            });
                        }
                    ...
                    error: function(xhr) {
                        ...
                        Toast.fire({
                            position: 'top-end',
                            title: xhr.responseJSON?.message ||
                                'An error occurred while deleting part from price list',
                            icon: "error"
                        });
                    }
                });
