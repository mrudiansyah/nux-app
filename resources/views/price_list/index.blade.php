...
@section('subcontent')
                                    ...
                                    <div class="d-flex justify-content-end" data-kt-goodreceive-table-toolbar="base">
                                        ...
                                        <div class="menu menu-sub menu-sub-dropdown w-300px w-md-325px"
                                            ...
                                            <div class="px-7 py-5">
                                                ...
                                                {{-- <div class="mb-5">
                                                    <label class="form-label fs-5 fw-bold mb-3">Range :</label>

                                                    <div class="row">
                                                        <div class="col-md-6">
                                                            <input type="date" class="form-control form-control-solid"
                                                                id="start_filter_date" placeholder="Start Date">
                                                        </div>

                                                        <div class="col-md-6">
                                                            <input type="date" class="form-control form-control-solid"
                                                                id="end_filter_date" placeholder="End Date">
                                                        </div>
                                                    </div>
                                                </div> --}}
                                        ...
                                        <button type="button" id="btnCreate" class="btn btn-light-primary btn-sm me-3">
                                            <span class="svg-icon svg-icon-2">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                    viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                                                    class="size-6">
                                ...
                                <table class="table align-middle table-row-dashed table-striped gy-2 fs-7"
                                    ...
                                    <thead>
                                        <tr class="text-start text-gray-900 fw-bolder fs-8 text-uppercase gs-0">
                                            ...
                                            <th width="25%" align="center">View</th>
                                        </tr>
                                    </thead>
                                    <tfoot>
                                        <tr class="text-start text-gray-900 fw-bolder fs-8 text-uppercase gs-0">
    ...
    <div class="modal fade" id="dualModal" tabindex="-1">
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                ...
                <div class="modal-header border-0 pb-0">
                    <ul class="nav nav-tabs" id="iframeTab">
                        <li class="nav-item">
                            <a class="nav-link active" href="javascript:void(0)"
                                onclick="switchIframe('pdf')">Confirmation Letter</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" href="javascript:void(0)" onclick="switchIframe('detail')">Price
                                Material</a>
                        </li>
                    </ul>
                </div>
                <div class="modal-body">
                    <iframe id="main_iframe" style="width:100%; height:500px; border:1px solid #ccc;">
                    </iframe>
                </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            price_list_table = $("#price_list_table").DataTable({
                ...
                info: false,
                ajax: {
                    url: "{{ url('price_list/show_price_list') }}",
                    type: 'post',
                    data: function(d) {
                        d._token = '{{ csrf_token() }}',
        ...
        $("#btnCreate").on('click', function() {
            const ref_doc = 'create'
            document_preview(ref_doc)
        })

        function document_preview(ref_doc) {
            window.history.replaceState({}, '', `<?php echo env('BASE_URL'); ?>/price_list?ref_doc=${ref_doc}`);
            $.ajax({
                url: '{{ url('price_list/create_view') }}',
                type: "post",
                data: {
                    _token: "{{ csrf_token() }}",
                    ref_doc: ref_doc
        ...
        function priceListDelete(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ url('price_list/delete_header') }}",
                        type: "post",
                        data: {
                            _token: "{{ csrf_token() }}",
                            id: id
        ...
        function openDualModal(id) {
            currentId = id;
            let pdfUrl = "{{ url('price_list/confirmation_letter') }}/" + id;
            $("#main_iframe").attr("src", pdfUrl);
            setActiveTab('pdf');

            $("#dualModal").modal("show");
        }
        ...
        function switchIframe(type) {
            ...
            if (type === 'pdf') {
                url = "{{ url('price_list/confirmation_letter') }}/" + currentId;
            } else if (type === 'detail') {
                url = "{{ url('price_list/price_material') }}/" + currentId;
            }
        ...
        function setActiveTab(type) {
            $("#iframeTab .nav-link").removeClass("active");

            if (type === 'pdf') {
                $("#iframeTab .nav-link").eq(0).addClass("active");
            } else {
                $("#iframeTab .nav-link").eq(1).addClass("active");
            }
        }
    </script>
@endsection
